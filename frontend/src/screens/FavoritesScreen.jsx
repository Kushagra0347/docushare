...
function FavoritesScreen({ className }) {
  ...
  const [sortBy, setSortBy] = useState(
    localStorage.getItem('favorites-sortBy') || 0,
  )
  ...
  function handleSortByChange(event) {
    ...
    localStorage.setItem('favorites-sortBy', event.target.value)
  }
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between">
        ...
        <div>
          {/* Heading */}
          <h2 className="mb-2 text-3xl font-bold">Favorites</h2>
          {/* Sort By Section */}
          {/* {files.length > 1 ? ( */}
          <div>
            <select
      ...
      <div className="mt-4 flex flex-wrap items-start justify-evenly">
        ...
        {loading ? (
          ...
          files?.map(
            (file, idx) =>
              file.starred &&
              file.uploaded_by_user.email == userInfo.email && (
                <FileCard
                  key={file.id}
